...
from ..repositories.resources import create_new, get_item_from_db, get_all_from_db, update_item_in_db
...
def get_all()->list[ResourceDetail]:
    ...
    return [ResourceDetail(name=item.name,description=item.description, id=item.id) for item in items_from_db]
